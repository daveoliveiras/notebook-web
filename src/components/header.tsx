...
export function Header(){
  ...
  return<header className='flex pl-8 pr-8 items-center bg-zinc-100 h-16'>
    ...
    <div className={`flex-auto ${useLocation().pathname.indexOf('edit') == -1 ? '' : 'hidden'} flex gap-1 ml-10`}>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger className='w-28 pl-3 pr-3 items-center justify-between flex rounded-sm bg-zinc-200 hover:bg-zinc-200/20 outline-none border'>
          {filter}
    ...
    <div className='flex flex-auto content-end justify-end'>
      <img src={UserPng} className='h-4 w-4'/>
      <span>Davi</span>
      <img src={LeavePng} className='h-4 w-4'/>
      <span>Sair</span>
    </div>
  </header>
}